...
- (void)startPhotoAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    if (self.selectedAssetArray.count>=self.maximumNumberOfSelection) {
        NSString  *str = [NSString stringWithFormat:@"最多选择%ld张照片",(long)self.maximumNumberOfSelection];
...
- (void)resetFinishFrame
{
    ...
    self.finishLabel.text = [NSString stringWithFormat:@"下一步(%ld)",(long)self.selectedAssetArray.count];
